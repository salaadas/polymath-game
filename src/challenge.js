...
function game_tick()
{
  ...
  if (cur_challenge_total < target_challenge)  spawn()
  ...
  if (input_idle < render_fps*input_idle_timeout)
  {
    ...
    const change_time = 4;
    if (gamespeed < goal_speed)  gamespeed *= (goal_speed/slow_speed) ** (1/(change_time*render_fps));
    else                         gamespeed = goal_speed;

    if (render_fps < goal_fps)  render_fps *= Math.pow(goal_fps/4, 1 / (2*render_fps));
    else                        render_fps  = goal_fps;
  }
  else
  {
    ...
    for (let k in c)
    {
      if (!c[k].dead)  maxx = Math.max(maxx, c[k].x);
    }
  ...
  if      (render_fps > goal_fps) render_fps = goal_fps;
  else if (render_fps < 2)        render_fps = 2;
}
